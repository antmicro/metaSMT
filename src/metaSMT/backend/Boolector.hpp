    ...
    class Boolector {
     ...
     public:
      ...
      struct BoolectorAssertion : public std::runtime_error {
        BoolectorAssertion(const char *what) : std::runtime_error(what) {}
      };

      static void _boolector_error(int) { throw BoolectorAssertion("internal error in boolector"); }
...
#define _bool_sort 1
      ...
      result_type operator()(predtags::false_tag, boost::any) { return ptr(boolector_false(_btor)); }

      result_type operator()(predtags::true_tag, boost::any) { return ptr(boolector_true(_btor)); }
      ...
      result_type operator()(bvtags::bit0_tag, boost::any) { return ptr(boolector_false(_btor)); }

      result_type operator()(bvtags::bit1_tag, boost::any) { return ptr(boolector_true(_btor)); }
      ...
      result_type operator()(TagT tag, result_type a, result_type b) {
        ...
        if (_has_key::value) {
          typedef
              typename mpl::eval_if<_has_key, mpl::at<Opcode_Map, TagT>, mpl::identity<Btor_F2<boolector_and> > >::type
                  opcode;
