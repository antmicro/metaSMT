...
namespace metaSMT {
  ...
  class SAT_Aiger {
   ...
   public:
    ...
    void assertion(result_type lit) { assertions.push_back(lit); }

    void assumption(result_type lit) { assumptions.push_back(lit); }
    ...
    void _eval(aiger_and const& and_sym) {
      ...
      if (evaluated.find(lhs) == evaluated.end()) {
        ...
        //           std::cout << "Sign: " << negated (lhs )<< " " << negated (rhs0) << " " << negated (rhs1) <<
        //           std::endl;
  ...
  namespace features {
    ...
    struct supports<SAT_Aiger<Context>, Feature> : supports<Context, Feature>::type {};
  }  // namespace features
