...
namespace metaSMT {
  ...
  namespace logic {
    ...
    namespace QF_UF {
      ...
      // real Grammar
      struct UninterpretedFunction_Grammar
          : proto::and_<proto::not_<proto::equal_to<proto::_, proto::_> >,
                        proto::or_<proto::nary_expr<tag::function_var_tag, proto::vararg<proto::_> > > > {};
      ...
      template <typename Expr>
      struct UninterpretedFunction : proto::extends<Expr, UninterpretedFunction<Expr>, UninterpretedFunction_Domain> {
        typedef proto::extends<Expr, UninterpretedFunction<Expr>, UninterpretedFunction_Domain> base_type;

        UninterpretedFunction(Expr const &expr = Expr()) : base_type(expr) {}
      };
      ...
      typedef proto::result_of::make_expr<proto::tag::terminal, UninterpretedFunction_Domain,
                                          tag::function_var_tag>::type Uninterpreted_Function;
      ...
      struct declare_function {
        ...
        declare_function &operator()() { return *this; }
      ...
      inline bool operator==(Uninterpreted_Function const &lhs, Uninterpreted_Function const &rhs) {
        ...
        if (lhs_tag.id != rhs_tag.id || !(lhs_tag.result_type == rhs_tag.result_type) ||
            lhs_tag.args.size() != rhs_tag.args.size()) {
          return false;
        }

        for (unsigned u = 0; u < lhs_tag.args.size(); ++u) {
          if (!(lhs_tag.args[u] == rhs_tag.args[u])) return false;
        }
