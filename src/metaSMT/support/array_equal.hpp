...
namespace metaSMT {
  namespace support {
    ...
    struct ArrayEqualBase {
      ArrayEqualBase(Lhs lhs, Rhs rhs, unsigned index_width) : lhs_(lhs), rhs_(rhs), index_width_(index_width) {}
    ...
    struct ArrayEqual : ArrayEqualBase<Lhs, Rhs> {
      ArrayEqual(Lhs lhs, Rhs rhs, unsigned index_width) : ArrayEqualBase<Lhs, Rhs>(lhs, rhs, index_width) {}
    };

    template <typename Lhs, typename Rhs>
    struct ArrayNequal : ArrayEqualBase<Lhs, Rhs> {
      ArrayNequal(Lhs lhs, Rhs rhs, unsigned index_width) : ArrayEqualBase<Lhs, Rhs>(lhs, rhs, index_width) {}
    };
  ...
  struct Evaluator<support::ArrayEqual<Lhs, Rhs> > : public boost::mpl::true_ {
    ...
    static typename Context::result_type eval(Context &ctx, support::ArrayEqual<Lhs, Rhs> const &c) {
      ...
      for (uint64_t index = 0; index < index_limit; ++index) {
        result = evaluate(
            ctx, And(result, equal(select(c.lhs_, bvuint(index, width)), select(c.rhs_, bvuint(index, width)))));
      }
  ...
  struct Evaluator<support::ArrayNequal<Lhs, Rhs> > : public boost::mpl::true_ {
    ...
    static typename Context::result_type eval(Context &ctx, support::ArrayNequal<Lhs, Rhs> const &c) {
      ...
      for (uint64_t index = 0; index < index_limit; ++index) {
        result = evaluate(
            ctx, Or(result, nequal(select(c.lhs_, bvuint(index, width)), select(c.rhs_, bvuint(index, width)))));
      }
